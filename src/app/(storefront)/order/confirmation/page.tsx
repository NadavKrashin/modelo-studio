  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center animate-fade-in">
      ...
      <div className="bg-white rounded-2xl border border-border/80 p-6 md:p-8 mb-8 max-w-sm mx-auto shadow-sm">
        ...
        <p className="text-3xl font-extrabold text-primary tracking-wide" dir="ltr">{orderNumber}</p>
      ...
      <div className="bg-muted-bg/60 rounded-2xl p-5 mb-8 text-start max-w-sm mx-auto">
        ...
        <ul className="space-y-2 text-sm text-muted">
          <li className="flex items-start gap-2">
            <span className="w-5 h-5 bg-primary text-white text-[10px] font-bold rounded-full flex items-center justify-center mt-0.5 shrink-0">1</span>
            <span>ההזמנה נבדקת ומאושרת על ידי הצוות</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="w-5 h-5 bg-primary/60 text-white text-[10px] font-bold rounded-full flex items-center justify-center mt-0.5 shrink-0">2</span>
            <span>המודל מודפס באיכות גבוהה</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="w-5 h-5 bg-primary/30 text-white text-[10px] font-bold rounded-full flex items-center justify-center mt-0.5 shrink-0">3</span>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <Link
          href={`/order/track?orderNumber=${encodeURIComponent(orderNumber)}${token ? `&token=${encodeURIComponent(token)}` : ''}`}
