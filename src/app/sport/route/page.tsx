...
export default function SportRoutePage() {
  ...
  const canGoNext = useMemo(() => {
    ...
    return true;
  }, [step, uploadedFile, raceName, participantName, distance]);
          ...
          <div className="rounded-2xl border border-slate-200 p-5 md:p-7">
            ...
            {step === 0 && (
              <div>
                ...
                <p className="text-slate-600 mb-5">העלו קובץ GPX מ-Strava/Garmin, או צילום מסך של המסלול (PNG/JPEG).</p>
            ...
            {step === 1 && (
              <div>
                ...
                <div className="space-y-4">
                  <input value={raceName} onChange={(e) => setRaceName(e.target.value)} className={inputCls} placeholder="שם המירוץ / אירוע" />
                  <input value={participantName} onChange={(e) => setParticipantName(e.target.value)} className={inputCls} placeholder="שם המשתתף" />
                  <input value={distance} onChange={(e) => setDistance(e.target.value)} className={inputCls} placeholder="מרחק (ק״מ)" />
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div>
                ...
                >
                  ...
                  <p className={`mt-1 text-sm ${wantsFrame ? "text-white/70" : "text-slate-500"}`}>הופך את המודל לפריט תצוגה מושלם</p>
                </button>
              </div>
            )}
